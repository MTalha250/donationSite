import Link from "next/link";
import React from "react";

const Hero = ({ title, image }: { title: string; image: string }) => {
  return (
    <div className="px-8 md:px-16 lg:px-24 xl:px-32 pt-32 h-[80vh]">
      <div className="flex h-full items-center border-b border-neutral-400 pb-20">
        <div className="w-full md:w-[65%]">
          <h1 className="text-2xl md:text-5xl font-semibold text-[#252525] leading-tight">
            {title}
          </h1>
          <p className="my-10 text-neutral-500 text-2xl">
            Help others by donating to their fundraiser, or start one for
            someone you care about.
          </p>
          <Link
            href="/fundraiser"
            className="bg-primary hover:bg-primary-dark text-lg py-4 px-8 rounded-md text-white"
          >
            Start a Fundraiser
          </Link>
        </div>
        <div className="pl-10 w-[35%] h-full">
          <img
            src={image}
            alt=""
            className="w-full h-full object-cover rounded-2xl"
          />
        </div>
      </div>
    </div>
  );
};

export default Hero;
